...
//nvkhoa created on 06 February 2009
function ob_edc(widget) {
    //enable and disable checkoxs
       //alert(widget);
       var ctl, j = 0;
       var arr = new Array(4);
        ctl = document.getElementsByTagName("input");
        for (var i=0; i<ctl.length; i++) {
		    if ((ctl[i].id.substr(0,4)=="chk_") && (ctl[i].checked) && (ctl[i].id.substr(0,6)== widget.substr(0,6))) {
		        arr[j] = ctl[i].id;
			    j = j + 1;
		    }
	    }
	    for (var k = 0; k < j; k++) {
	        for (var t=0; t<ctl.length; t++) {
	            if ((ctl[t].id.substr(0,4)=="chk_") && ctl[t].id == arr[k] || ctl[t].checked == true) {
    	            ctl[t].disabled = false;
	            } else {
	                if (ctl[t].id.substr(0,6)== widget.substr(0,6)) {
	                    ctl[t].disabled = true;
	                }
	            }
	        }
...
//nvkhoa updated on 05 February 2009
function ob_t2c(e) {
	...
	if(e.checked==false) {
		...
		ob_t2_unchk_parent(e.parentNode, e.id);
	...
	if(ob_hasChildren(e.parentNode)) {
	    e.disabled = false;
		var ob_t2in1=e.parentNode.parentNode.parentNode.parentNode.nextSibling.getElementsByTagName("input");
		//alert(ob_t2in1.length);
		for (var i=0; i<ob_t2in1.length; i++) {
			
				ob_t2in[i].disabled = false;
			
		}
	}
	
//	var ob_t2in1,ob_t2s1,ob_t2checked1="";
//	ob_t2in1=document.getElementsByTagName("input");
//	for (var i=0; i<ob_t2in1.length; i++) {
//		ob_t2s1=ob_t2in1[i].id;
//		if ((ob_t2s1.substr(0,5)=="chk_1") && (ob_t2in1[i].checked)) {
//			ob_t2checked1=ob_t2checked1+ob_t2s+",";
//		}
//	}
//	return ob_t2checked1;
}
...
//nvkhoa
function ob_t2_unchk_parent(e, widget) {
    ...
    ctl = document.getElementsByTagName("input");
    for (var i=0; i<ctl.length; i++) {
		if ((ctl[i].id.substr(0,6)==e.id.substr(0,6)) && (ctl[i].checked)) {
		    arr[j] = ctl[i].id;
			j = j + 1;
		}
	}
